...
def get_formatted_scans_key_row(dcm_fn: str | Path) -> list[str]:
    ...
    # acq_datetime = dicoms.get_datetime_from_dcm(dcm_data=dcm_data)
     #using Dicom header tag "Study Date" instead, because Acquisition Date and Time not always present (e.g. in annonymized dicom headers)
    acq_datetime = datetime.datetime.strptime(dcm_data.StudyDate, "%Y-%m-%d %H:%M:%S")
    # add random string
