...
def infotodict(seqinfo):
    ...
    info = {t1w_3d: [], t1w_ax: [], t1w_sag: [], t1w_cor: [],  t2w_3d: [], t2w_2d: [], flair_3d: [], flair_ax: [], flair_sag: [], flair_cor: []}

    print("Tshooting heuristic.py, AcquisitionType issue::  ")
    print(seqinfo)
    ## TODO Account for missing tags not present
    ...
    for s in seqinfo:
        ...
        if (s.dim4 == 1) and ('ORIGINAL' in s.image_type) and ('loc' not in s.series_description.lower()) and ('tof' not in s.series_description.lower()) and ('post' not in s.series_description.lower()) and ('repeat' not in s.series_description.lower()) and ('2D' in s.AcquisitionType) and ('ax' in s.series_description.lower()) and ('flair' in s.series_description.lower()):
           info[flair_ax].append(s.series_id) 
        if (s.dim4 == 1) and ('ORIGINAL' in s.image_type) and ('loc' not in s.series_description.lower()) and ('tof' not in s.series_description.lower()) and ('post' not in s.series_description.lower()) and ('repeat' not in s.series_description.lower()) and ('2D' in s.AcquisitionType) and ('sag' in s.series_description.lower()) and ('flair' in s.series_description.lower()):
           info[flair_sag].append(s.series_id) 
        if (s.dim4 == 1) and ('ORIGINAL' in s.image_type) and ('loc' not in s.series_description.lower()) and ('tof' not in s.series_description.lower()) and ('post' not in s.series_description.lower()) and ('repeat' not in s.series_description.lower()) and ('2D' in s.AcquisitionType) and ('cor' in s.series_description.lower()) and ('flair' in s.series_description.lower()):
           info[flair_cor].append(s.series_id)    
